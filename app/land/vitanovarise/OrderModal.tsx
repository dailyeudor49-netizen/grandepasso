...
const COLORS = [
  { id: "bianca", label: "Bianca", hex: "#FFFFFF", border: true },
  { id: "nero", label: "Nero", hex: "#1a1a1a" },
  { id: "grigio", label: "Grigio", hex: "#808080" },
  { id: "blu", label: "Blu", hex: "#1e3a8a" },
  { id: "giallo-fluo", label: "Giallo Fluo", hex: "#ccff00" },
  { id: "arancione-papaya", label: "Arancione Papaya", hex: "#ff6f3c" },
];

const SIZE_TO_FULLSHIP: Record<string, number> = {
  "36": 5943, "37": 5944, "38": 5945, "39": 5946,
  "40": 5947, "41": 5948, "42": 5949, "43": 5950, "44": 5951, "45": 5952,
};
...
export function OrderSection({ image }: { image: string }) {
  ...
  const [color, setColor] = useState("bianca");
  ...
  const handleSubmit = async () => {
    ...
    let orderToken = "";
    try {
      const tokenRes = await fetch("/api/order-token?slug=vitanovarise");
      if (tokenRes.ok) {
    ...
    try {
      ...
      recordOrder();
      window.location.href = "/land/vitanovastep/ty";
    } catch (err: unknown) {
          ...
          >
              ...
              <div className="px-5 pt-4 pb-5">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[15px] font-semibold text-[#0e7490]">Plantare Ortopedico</p>
            ...
            {step === 2 && (
              <div className="px-5 pt-4 pb-5">
                ...
                {!showEmail ? (
                  <button type="button" onClick={() => setShowEmail(true)} className="flex items-center gap-1.5 text-[13px] font-semibold text-[#e87a00] mb-3.5 cursor-pointer bg-transparent border-none p-0">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-[#e87a00]"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
                    Aggiungi email (facoltativo)
                  </button>
                ) : (
                  <div className="mb-3.5">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[13px] font-semibold text-gray-700">Email <span className="text-xs font-normal text-[#b87a00]">(facoltativo)</span></label>
                      <button type="button" onClick={() => { setShowEmail(false); updateForm("email", ""); }} className="flex items-center gap-1 text-[11px] font-semibold text-[#e87a00] cursor-pointer bg-transparent border-none p-0">
                        <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-[#e87a00]"><path d="M19 13H5v-2h14v2z"/></svg>
                        Rimuovi
                      </button>
                    </div>
                ...
                {!showNotes ? (
                  <button type="button" onClick={() => setShowNotes(true)} className="flex items-center gap-1.5 text-[13px] font-semibold text-[#e87a00] mb-3.5 cursor-pointer bg-transparent border-none p-0">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-[#e87a00]"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
                  ...
                  <div className="mb-3.5">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[13px] font-semibold text-gray-700">Note per il corriere <span className="text-xs font-normal text-[#b87a00]">(facoltativo)</span></label>
                      <button type="button" onClick={() => { setShowNotes(false); updateForm("shippingNotes", ""); }} className="flex items-center gap-1 text-[11px] font-semibold text-[#e87a00] cursor-pointer bg-transparent border-none p-0">
                        <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-[#e87a00]"><path d="M19 13H5v-2h14v2z"/></svg>
                        Rimuovi
                      </button>
                    </div>
                    <textarea placeholder="Es. citofono non funzionante, lasciare al vicino..." rows={2} value={form.shippingNotes} onChange={(e) => updateForm("shippingNotes", e.target.value)} className="w-full py-3 px-3.5 border-[1.5px] border-gray-300 rounded-[10px] text-sm text-gray-700 bg-white outline-none transition-colors resize-y min-h-[56px] focus:border-[#e87a00] focus:shadow-[0_0_0_3px_rgba(232,122,0,0.1)]" />
                  </div>
                )}
            ...
            {step === "phone-blocked" && (
              <div className="text-center py-[30px] px-5">
                <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: "linear-gradient(135deg, #e87a00, #c66a00)" }}>
                ...
                <h3 className="text-lg font-extrabold text-[#c66a00] mb-2">Hai già effettuato un ordine</h3>
                <p className="text-sm text-gray-500 mb-5 leading-relaxed">
                  Risulta già un ordine con questo numero di telefono. Per assistenza o per effettuare un nuovo ordine, contattaci.
                </p>
                <a href="/contatti" className="inline-block py-3.5 px-8 text-white rounded-full text-[15px] font-semibold cursor-pointer w-full text-center" style={{ background: "linear-gradient(to bottom, #f0a030, #d48806)" }}>
                  Contattaci
                </a>
