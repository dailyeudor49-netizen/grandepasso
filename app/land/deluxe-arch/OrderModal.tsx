...
const SIZE_TO_FULLSHIP: Record<string, number> = {
  "36": 5974, "37": 5975, "38": 5976, "39": 5977,
  "40": 5978, "41": 5979, "42": 5980, "43": 5981,
};
...
export function OrderSection({ image }: { image: string }) {
  ...
  const handleSubmit = async () => {
    ...
    let orderToken = "";
    try {
      const tokenRes = await fetch("/api/order-token?slug=deluxe-arch");
      if (tokenRes.ok) {
    ...
    try {
      ...
      recordOrder();
      window.location.href = "/land/deluxe-arch/ty";
    } catch (err: unknown) {
              ...
              <div className="px-5 pt-4 pb-5">
                ...
                <div className="border-t border-gray-200 mt-[18px] pt-3.5 mb-4">
                  <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
                    <span>Sandali</span><span className="font-medium">{fmtPrice(PRODUCT_PRICE)}</span>
                  </div>
                  {upsell && (
                    <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
