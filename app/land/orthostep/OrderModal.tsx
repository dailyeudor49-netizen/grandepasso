...
const PRODUCT_TITLE = "OrthoStep Pro";
const PRODUCT_PRICE = 49.90;
const PRODUCT_COMPARE = 99.90;
const SHOP_NAME = "vitanova";

const COLORS = ["Nero", "Bianco", "Beige"];
const COLOR_HEX: Record<string, string> = { "Nero": "#1a1a1a", "Bianco": "#f5f5f5", "Beige": "#d4b896" };
const COLOR_IMAGES: Record<string, string> = {
  "Nero": "/images/land/orthostep/carosello/nera.png",
  "Bianco": "/images/land/orthostep/carosello/bianco.png",
  "Beige": "/images/land/orthostep/carosello/beige.png",
};
...
export function OrderSection({ image, onColorChange }: { image: string; onColorChange?: (img: string) => void }) {
  const [color, setColor] = useState(COLORS[0]);
  const [size, setSize] = useState("40");

  // Immagine dinamica basata sul colore selezionato
  const currentImage = COLOR_IMAGES[color] || image;

  // Callback per aggiornare immagine esterna
  const handleColorChange = (newColor: string) => {
    setColor(newColor);
    const newImage = COLOR_IMAGES[newColor] || image;
    if (onColorChange) onColorChange(newImage);
  };
  ...
  const handleSubmit = async () => {
    ...
    let orderToken = "";
    try {
      const tokenRes = await fetch("/api/order-token?slug=orthostep");
      if (tokenRes.ok) {
    ...
    try {
      ...
      recordOrder();
      window.location.href = "/land/orthostep/ty";
    } catch (err: unknown) {
          ...
          >
              ...
              <div className="px-5 pt-4 pb-5">
                ...
                <div className="mb-3.5">
                  {!showEmail ? (
                    <button
                      ...
                      className="flex items-center gap-1.5 text-[13px] font-semibold text-[#e87a00] cursor-pointer"
                    >
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <button
                          type="button"
                          onClick={() => { setShowEmail(false); updateForm("email", ""); }}
                          className="text-[11px] text-red-400 font-semibold cursor-pointer"
                        >
                          Rimuovi
                        </button>
                      </div>
                ...
                <div className="mb-3.5">
                  {!showNotes ? (
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="block text-[13px] font-semibold text-gray-700">Note per il corriere</label>
                        <button
                          type="button"
                          onClick={() => { setShowNotes(false); updateForm("shippingNotes", ""); }}
                          className="text-[11px] text-red-400 font-semibold cursor-pointer"
                        >
                          Rimuovi
                        </button>
            ...
            {step === "blocked" && (
              <div className="text-center py-[30px] px-5">
                ...
                <p className="text-sm text-gray-500 mb-5 leading-relaxed">Hai gi&agrave; effettuato un ordine su questo sito. Non &egrave; possibile effettuare ulteriori ordini.</p>
            ...
            {step === "phone-blocked" && (
              <div className="text-center py-[30px] px-5">
                <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: "linear-gradient(135deg, #f0a030, #d48806)" }}>
                  <svg viewBox="0 0 24 24" className="w-8 h-8 stroke-white fill-none" strokeWidth={3}><path d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636" /></svg>
                </div>
                <h3 className="text-lg font-extrabold text-[#d48806] mb-2">Hai gi&agrave; effettuato un ordine</h3>
                <p className="text-sm text-gray-500 mb-5 leading-relaxed">Risulta gi&agrave; un ordine associato a questo numero di telefono. Per assistenza o modifiche, visita la nostra pagina contatti.</p>
                <a href="/contatti" className="inline-block py-3.5 px-8 rounded-full text-white text-[15px] font-semibold cursor-pointer w-full text-center" style={{ background: "linear-gradient(to bottom, #f0a030, #d48806)" }}>
                  Vai ai Contatti
                </a>
              </div>
            )}
