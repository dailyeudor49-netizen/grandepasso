...
const COLORS = [
  { id: "rosa", label: "Rosa", hex: "#e8a0b5", border: false },
...
export function OrderSection({ image }: { image: string }) {
  ...
  const handleSubmit = async () => {
    ...
    let orderToken = "";
    try {
      const tokenRes = await fetch("/api/order-token?slug=spinair");
      if (tokenRes.ok) {
    ...
    try {
      ...
      localStorage.setItem("cf_thankyou", JSON.stringify({
        ...
        upsell: upsell ? { name: "Soletta Ortopedica Pro", price: UPSELL_PRICE.toFixed(2) } : null,
    ...
    try {
      ...
      recordOrder();
      window.location.href = "/land/spinair/ty";
    } catch (err: unknown) {
  ...
  const inputCls = (field: string) =>
    `w-full py-3 px-3.5 border-[1.5px] rounded-[10px] text-sm text-gray-700 bg-white outline-none transition-colors ${
      errors[field] ? "border-red-500 bg-red-50" : "border-gray-300 focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(139,92,246,0.1)]"
    }`;
  ...
  const violetBtnCls = "w-full py-4 rounded-full text-white text-base font-bold cursor-pointer transition-all flex items-center justify-center gap-2 shadow-[0_4px_16px_rgba(139,92,246,0.3)] hover:shadow-[0_6px_22px_rgba(139,92,246,0.4)]";
  const violetBg = { background: "linear-gradient(to bottom, #8b5cf6, #7c3aed)" };
    ...
    <>
      ...
      <div className="flex flex-col items-center w-full my-5" id="ordina">
        <button
          onClick={openModal}
          style={violetBg}
          className="inline-flex items-center justify-center gap-2.5 w-full max-w-[600px] px-9 py-[18px] rounded-full text-white text-lg font-bold border-none cursor-pointer transition-all shadow-[0_4px_20px_rgba(139,92,246,0.35)] hover:shadow-[0_8px_30px_rgba(139,92,246,0.45)] hover:-translate-y-0.5 active:translate-y-0"
        >
          <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current">
            <path d="M3 3h18v2H3zm0 4h18v14H3zm4 4h2v2H7zm4 0h2v2h-2zm4 0h2v2h-2zM7 15h10v2H7z" />
          </svg>
          Ordina Ora — Libera La Tua Schiena
        </button>
      ...
      {modalOpen && (
        ...
        >
          ...
          >
            ...
            {step !== "blocked" && step !== "phone-blocked" && (
              <div className="flex items-center justify-center gap-0 px-5 pt-4 pb-3">
                <div className="flex items-center">
                  <span className={`w-[26px] h-[26px] rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${step >= 1 ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-400"}`}>1</span>
                  <span className={`text-xs font-semibold ml-1.5 whitespace-nowrap ${step >= 1 ? "text-blue-500" : "text-gray-400"}`}>Riepilogo</span>
                </div>
                <div className={`flex-1 h-[3px] mx-2 rounded transition-colors ${step === 2 ? "bg-blue-500" : "bg-gray-200"}`} />
                <div className="flex items-center">
                  <span className={`w-[26px] h-[26px] rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${step === 2 ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-400"}`}>2</span>
                  <span className={`text-xs font-semibold ml-1.5 whitespace-nowrap ${step === 2 ? "text-blue-500" : "text-gray-400"}`}>Dati spedizione</span>
                </div>
              </div>
            )}
            ...
            {step === 1 && (
              <div className="px-5 pt-4 pb-5">
                ...
                <div className="flex gap-3.5 items-center p-3.5 bg-[#fafafa] border border-gray-200 rounded-xl mb-4">
                  ...
                  <span onClick={closeModal} className="text-xs text-blue-500 font-semibold cursor-pointer underline shrink-0 self-start mt-0.5">Modifica</span>
                </div>
                ...
                <div
                  ...
                  className={`flex items-start gap-3 p-3.5 border-2 rounded-xl cursor-pointer transition-all select-none mb-5 flex-wrap ${
                    upsell ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-500 hover:bg-blue-50"
                  }`}
                >
                  <div className={`w-[22px] h-[22px] border-2 rounded-[5px] flex items-center justify-center shrink-0 mt-0.5 transition-all ${
                    upsell ? "bg-blue-500 border-blue-500" : "border-gray-300"
                  }`}>
                  ...
                  <div className="w-16 h-16 rounded-lg border border-gray-200 overflow-hidden shrink-0 bg-[#fafafa]">
                    <img src="https://www.otomedical.it/wp-content/uploads/2023/01/plantari-ortopedici.png" alt="Soletta Ortopedica Pro" className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-gray-900 mb-px">Aggiungi al tuo ordine</p>
                    <p className="text-[15px] font-semibold text-blue-600">Soletta Ortopedica Pro</p>
                    <p className="text-[13px] text-gray-400 leading-snug mb-1.5">Potenzia il supporto lombare. Distribuzione ottimale del peso.</p>
                ...
                {/* Proceed */}
                <button onClick={() => goToStep(2)} style={violetBg} className={violetBtnCls}>
            ...
            {step === 2 && (
              <div className="px-5 pt-4 pb-5">
                ...
                <div className="mb-[18px] flex flex-col gap-2.5">
                  ...
                  {upsell && (
                    <div className="flex gap-3 items-center p-3 bg-[#fafafa] border border-gray-200 rounded-[10px]">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-gray-900 leading-tight">Soletta Ortopedica Pro</p>
                ...
                <div className="mb-3.5">
                  ...
                  <div className={`flex items-stretch border-[1.5px] rounded-[10px] overflow-hidden transition-colors ${errors.phoneNumber ? "border-red-500 bg-red-50" : "border-gray-300 focus-within:border-blue-500 focus-within:shadow-[0_0_0_3px_rgba(139,92,246,0.1)]"}`}>
                ...
                {!showEmail ? (
                  <button type="button" onClick={() => setShowEmail(true)} className="flex items-center gap-1.5 text-[13px] font-semibold text-blue-500 mb-3.5 cursor-pointer bg-transparent border-none p-0">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-blue-500"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
                    Aggiungi email (facoltativo)
                  </button>
                ) : (
                  <div className="mb-3.5">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[13px] font-semibold text-gray-700">Email <span className="text-xs font-normal text-blue-500">(facoltativo)</span></label>
                      <button type="button" onClick={() => { setShowEmail(false); updateForm("email", ""); }} className="flex items-center gap-1 text-[11px] font-semibold text-blue-500 cursor-pointer bg-transparent border-none p-0">
                        <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-blue-500"><path d="M19 13H5v-2h14v2z"/></svg>
                        Rimuovi
                      </button>
                    </div>
                ...
                {!showNotes ? (
                  <button type="button" onClick={() => setShowNotes(true)} className="flex items-center gap-1.5 text-[13px] font-semibold text-blue-500 mb-3.5 cursor-pointer bg-transparent border-none p-0">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-blue-500"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
                  ...
                  <div className="mb-3.5">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[13px] font-semibold text-gray-700">Note per il corriere <span className="text-xs font-normal text-blue-500">(facoltativo)</span></label>
                      <button type="button" onClick={() => { setShowNotes(false); updateForm("shippingNotes", ""); }} className="flex items-center gap-1 text-[11px] font-semibold text-blue-500 cursor-pointer bg-transparent border-none p-0">
                        <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-blue-500"><path d="M19 13H5v-2h14v2z"/></svg>
                        Rimuovi
                      </button>
                    </div>
                    <textarea placeholder="Es. citofono non funzionante, lasciare al vicino..." rows={2} value={form.shippingNotes} onChange={(e) => updateForm("shippingNotes", e.target.value)} className="w-full py-3 px-3.5 border-[1.5px] border-gray-300 rounded-[10px] text-sm text-gray-700 bg-white outline-none transition-colors resize-y min-h-[56px] focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(139,92,246,0.1)]" />
                  </div>
                )}
                ...
                <div className="border-t border-gray-200 mt-[18px] pt-3.5 mb-4">
                  ...
                  {upsell && (
                    <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
                      <span>Soletta Ortopedica Pro</span><span className="font-medium">{fmtPrice(UPSELL_PRICE)}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
                ...
                {/* COD warning */}
                <div className="bg-blue-50 border-[1.5px] border-violet-200 rounded-[10px] p-3.5 mb-[18px] text-xs text-violet-800 leading-relaxed">
            ...
            {step === "phone-blocked" && (
              <div className="text-center py-[30px] px-5">
                <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: "linear-gradient(135deg, #8b5cf6, #7c3aed)" }}>
                ...
                <h3 className="text-lg font-extrabold text-blue-600 mb-2">Hai già effettuato un ordine</h3>
                <p className="text-sm text-gray-500 mb-5 leading-relaxed">
                  Risulta già un ordine con questo numero di telefono. Per assistenza o per effettuare un nuovo ordine, contattaci.
                </p>
                <a href="/contatti" className="inline-block py-3.5 px-8 text-white rounded-full text-[15px] font-semibold cursor-pointer w-full text-center" style={{ background: "linear-gradient(to bottom, #8b5cf6, #7c3aed)" }}>
                  Contattaci
                </a>
