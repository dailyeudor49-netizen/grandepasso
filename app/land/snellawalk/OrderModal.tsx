...
const PRODUCT_TITLE = "SnellaWalk";
...
function getSpamState(): { status: "free" | "blocked" } {
  try { if (localStorage.getItem(STORE_KEY) === "1") return { status: "blocked" }; } catch {}
  return { status: "free" };
}
...
export function OrderSection({ image }: { image: string }) {
  ...
  const handleSubmit = async () => {
    ...
    let orderToken = "";
    try {
      const tokenRes = await fetch("/api/order-token?slug=snellawalk");
      if (tokenRes.ok) {
    ...
    try {
      ...
      window.location.href = "/land/snellawalk/ty";
    } catch (err: unknown) {
          ...
          >
            ...
            {step === "phone-blocked" && (
              <div className="text-center py-[30px] px-5">
                ...
                <p className="text-sm text-gray-500 mb-5 leading-relaxed">Risulta già un ordine con questo numero di telefono. Per assistenza o per effettuare un nuovo ordine, contattaci.</p>
                <a href="/contatti" className="inline-block py-3.5 px-8 text-white rounded-full text-[15px] font-semibold cursor-pointer w-full text-center" style={{ background: "linear-gradient(to bottom, #f0a030, #d48806)" }}>Contattaci</a>
