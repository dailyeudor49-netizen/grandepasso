...
const SIZE_TO_FULLSHIP: Record<string, number> = {
  "36": 6074, "37": 6075, "38": 6076, "39": 6077,
  "40": 6078, "41": 6079, "42": 6080, "43": 6081,
};
...
const COLORS = [
  { name: "Taupe", value: "taupe", hex: "#B8A99A" },
  { name: "Nero", value: "black", hex: "#1A1A1A" },
];
...
export function OrderSection({ image }: { image: string }) {
  ...
  const handleSubmit = async () => {
    ...
    let orderToken = "";
    try {
      const tokenRes = await fetch("/api/order-token?slug=pandora");
      if (tokenRes.ok) {
    ...
    try {
      ...
      localStorage.setItem("cf_thankyou", JSON.stringify({
        ...
        timestamp: Date.now(),
        gadsConv: "AW-17101926572/RYRNCPiXnqAcEKzh6to_",
        _payload: payload,
      }));
    ...
    try {
      ...
      recordOrder();
      window.location.href = "/land/pandora/ty";
    } catch (err: unknown) {
              ...
              <div className="px-5 pt-4 pb-5">
                ...
                <div className="border-t border-gray-200 mt-[18px] pt-3.5 mb-4">
                  <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
                    <span>Ciabatte Platform</span><span className="font-medium">{fmtPrice(PRODUCT_PRICE)}</span>
                  </div>
                  {upsell && (
                    <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
