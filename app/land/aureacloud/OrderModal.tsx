...
const PRODUCT_TITLE = "AureaCloud";
...
const SIZE_TO_FULLSHIP: Record<string, number> = {
  "36": 6001, "37": 6002, "38": 6003, "39": 6004,
  "40": 6005, "41": 6006, "42": 6007, "43": 6008, "44": 6009,
};
...
export function OrderSection({ image }: { image: string }) {
  ...
  const handleSubmit = async () => {
    ...
    let orderToken = "";
    try {
      const tokenRes = await fetch("/api/order-token?slug=aureacloud");
      if (tokenRes.ok) {
    ...
    try {
      ...
      localStorage.setItem("cf_thankyou", JSON.stringify({
        ...
        upsell: upsell ? { name: "Charm Pack Decorativi", price: UPSELL_PRICE.toFixed(2) } : null,
    ...
    try {
      ...
      recordOrder();
      window.location.href = "/land/aureacloud/ty";
    } catch (err: unknown) {
  ...
  const inputCls = (field: string) =>
    `w-full py-3 px-3.5 border-[1.5px] rounded-[10px] text-sm text-gray-700 bg-white outline-none transition-colors ${
      errors[field] ? "border-red-500 bg-red-50" : "border-gray-300 focus:border-[#ec4899] focus:shadow-[0_0_0_3px_rgba(236,72,153,0.1)]"
    }`;
          ...
          >
            ...
            {step !== "blocked" && step !== "phone-blocked" && (
              <div className="flex items-center justify-center gap-0 px-5 pt-4 pb-3">
                <div className="flex items-center">
                  <span className={`w-[26px] h-[26px] rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${step >= 1 ? "bg-[#ec4899] text-white" : "bg-gray-200 text-gray-400"}`}>1</span>
                  <span className={`text-xs font-semibold ml-1.5 whitespace-nowrap ${step >= 1 ? "text-[#ec4899]" : "text-gray-400"}`}>Riepilogo</span>
                </div>
                <div className={`flex-1 h-[3px] mx-2 rounded transition-colors ${step === 2 ? "bg-[#ec4899]" : "bg-gray-200"}`} />
                <div className="flex items-center">
                  <span className={`w-[26px] h-[26px] rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${step === 2 ? "bg-[#ec4899] text-white" : "bg-gray-200 text-gray-400"}`}>2</span>
                  <span className={`text-xs font-semibold ml-1.5 whitespace-nowrap ${step === 2 ? "text-[#ec4899]" : "text-gray-400"}`}>Dati spedizione</span>
                </div>
              </div>
            )}
            ...
            {step === 1 && (
              <div className="px-5 pt-4 pb-5">
                {/* Product card */}
                <div className="flex gap-3.5 items-center p-3.5 bg-[#fdf2f8] border border-pink-200 rounded-xl mb-4">
                  <div className="w-[72px] h-[72px] rounded-[10px] border border-pink-200 overflow-hidden shrink-0 bg-white">
                  ...
                  <span onClick={closeModal} className="text-xs text-[#ec4899] font-semibold cursor-pointer underline shrink-0 self-start mt-0.5">Modifica</span>
                </div>
                ...
                <div
                  ...
                  className={`flex items-start gap-3 p-3.5 border-2 rounded-xl cursor-pointer transition-all select-none mb-5 flex-wrap ${
                    upsell ? "border-[#ec4899] bg-[#fdf2f8]" : "border-gray-200 hover:border-[#ec4899] hover:bg-[#fdf2f8]"
                  }`}
                >
                  <div className={`w-[22px] h-[22px] border-2 rounded-[5px] flex items-center justify-center shrink-0 mt-0.5 transition-all ${
                    upsell ? "bg-[#ec4899] border-[#ec4899]" : "border-gray-300"
                  }`}>
                  ...
                  <div className="w-16 h-16 rounded-lg border border-gray-200 overflow-hidden shrink-0 bg-[#fafafa]">
                    <img src="https://ae01.alicdn.com/kf/Sb0feee15b6cd4e4b8b3a44c75ed9bab7M/10Pcs-Cartoon-Shoe-Croc-Charms-Animals-Cute-Dog-Cat-Rabbit-Bear-Shoe-Decoration-Kids-Shoes-Buckle.jpg" alt="Charm Pack" className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-gray-900 mb-px">Aggiungi al tuo ordine</p>
                    <p className="text-[15px] font-semibold text-[#ec4899]">Pack 10 Charm Decorativi</p>
                    <p className="text-[13px] text-gray-400 leading-snug mb-1.5">Personalizza le tue AureaCloud con charm kawaii esclusivi.</p>
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm text-gray-400 line-through">14,99 &euro;</span>
                      <span className="text-[17px] font-bold text-emerald-600">+4,99 &euro;</span>
                    </div>
                  </div>
                </div>
            ...
            {step === 2 && (
              <div className="px-5 pt-4 pb-5">
                ...
                <div className="mb-[18px] flex flex-col gap-2.5">
                  <div className="flex gap-3 items-center p-3 bg-[#fdf2f8] border border-pink-200 rounded-[10px]">
                    <div className="w-[60px] h-[60px] rounded-lg border border-pink-200 overflow-hidden shrink-0 bg-white">
                      <img src={currentImage} alt="" className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                  ...
                  {upsell && (
                    <div className="flex gap-3 items-center p-3 bg-[#fdf2f8] border border-pink-200 rounded-[10px]">
                      <div className="w-[60px] h-[60px] rounded-lg border border-pink-200 overflow-hidden shrink-0 bg-[#fafafa]">
                        <img src="https://ae01.alicdn.com/kf/Sb0feee15b6cd4e4b8b3a44c75ed9bab7M/10Pcs-Cartoon-Shoe-Croc-Charms-Animals-Cute-Dog-Cat-Rabbit-Bear-Shoe-Decoration-Kids-Shoes-Buckle.jpg" alt="Charm Pack" className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-gray-900 leading-tight">Pack 10 Charm Decorativi</p>
                        <p className="text-[11px] text-gray-400">Charm kawaii esclusivi</p>
                      </div>
                ...
                <div className="mb-3.5">
                  ...
                  <div className={`flex items-stretch border-[1.5px] rounded-[10px] overflow-hidden transition-colors ${errors.phoneNumber ? "border-red-500 bg-red-50" : "border-gray-300 focus-within:border-[#ec4899] focus-within:shadow-[0_0_0_3px_rgba(236,72,153,0.1)]"}`}>
                ...
                {!showEmail ? (
                  <button type="button" onClick={() => setShowEmail(true)} className="flex items-center gap-1.5 text-[13px] font-semibold text-[#ec4899] mb-3.5 cursor-pointer bg-transparent border-none p-0">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-[#ec4899]"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
                    Aggiungi email (facoltativo)
                  </button>
                ) : (
                  <div className="mb-3.5">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[13px] font-semibold text-gray-700">Email <span className="text-xs font-normal text-[#ec4899]">(facoltativo)</span></label>
                      <button type="button" onClick={() => { setShowEmail(false); updateForm("email", ""); }} className="flex items-center gap-1 text-[11px] font-semibold text-[#ec4899] cursor-pointer bg-transparent border-none p-0">
                        <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-[#ec4899]"><path d="M19 13H5v-2h14v2z"/></svg>
                        Rimuovi
                      </button>
                    </div>
                ...
                {!showNotes ? (
                  <button type="button" onClick={() => setShowNotes(true)} className="flex items-center gap-1.5 text-[13px] font-semibold text-[#ec4899] mb-3.5 cursor-pointer bg-transparent border-none p-0">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-[#ec4899]"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
                  ...
                  <div className="mb-3.5">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[13px] font-semibold text-gray-700">Note per il corriere <span className="text-xs font-normal text-[#ec4899]">(facoltativo)</span></label>
                      <button type="button" onClick={() => { setShowNotes(false); updateForm("shippingNotes", ""); }} className="flex items-center gap-1 text-[11px] font-semibold text-[#ec4899] cursor-pointer bg-transparent border-none p-0">
                        <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-[#ec4899]"><path d="M19 13H5v-2h14v2z"/></svg>
                        Rimuovi
                      </button>
                    </div>
                    <textarea placeholder="Es. citofono non funzionante, lasciare al vicino..." rows={2} value={form.shippingNotes} onChange={(e) => updateForm("shippingNotes", e.target.value)} className="w-full py-3 px-3.5 border-[1.5px] border-gray-300 rounded-[10px] text-sm text-gray-700 bg-white outline-none transition-colors resize-y min-h-[56px] focus:border-[#ec4899] focus:shadow-[0_0_0_3px_rgba(236,72,153,0.1)]" />
                  </div>
                )}
                ...
                <div className="border-t border-gray-200 mt-[18px] pt-3.5 mb-4">
                  <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
                    <span>Ciabatte AureaCloud</span><span className="font-medium">{fmtPrice(PRODUCT_PRICE)}</span>
                  </div>
                  {upsell && (
                    <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
                      <span>Pack 10 Charm</span><span className="font-medium">{fmtPrice(UPSELL_PRICE)}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center py-1 text-[13px] text-gray-500">
                ...
                {/* COD warning */}
                <div className="bg-[#fdf2f8] border-[1.5px] border-[#f9a8d4] rounded-[10px] p-3.5 mb-[18px] text-xs text-[#9d174d] leading-relaxed">
            ...
            {step === "phone-blocked" && (
              <div className="text-center py-[30px] px-5">
                ...
                <h3 className="text-lg font-extrabold text-[#db2777] mb-2">Hai già effettuato un ordine</h3>
